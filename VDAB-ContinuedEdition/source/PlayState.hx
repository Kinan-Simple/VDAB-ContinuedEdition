	...
	override public function create()
	{
		...
		case 'houseOlderDay': //Older Dave Week
			...
			case 'oldRed':
			{
				defaultCamZoom = 0.85;
				curStage = 'oldlel';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('oldlel/sussus'));
			...
			case 'bensonbg':
			{
				defaultCamZoom = 0.85;
				curStage = 'bensonbg';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('extrabg/bensonbg'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.6, 0.6);
				bg.active = true;
		...
		case 'death':
			{
				...
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('extrabg/sky'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.active = true;	

				var hills:FlxSprite = new FlxSprite(-300, 110).loadGraphic(Paths.image('extrabg/brick'));
				hills.antialiasing = true;
				hills.scrollFactor.set(0.5, 0.5);
				hills.active = true;

				var foreground:FlxSprite = new FlxSprite(-400, 600).loadGraphic(Paths.image('extrabg/complicated_pain'));
				foreground.antialiasing = true;
				foreground.scrollFactor.set(1, 1);
				foreground.active = true;

				var cornSet:FlxSprite = new FlxSprite(-350, 325).loadGraphic(Paths.image('extrabg/deadcorn'));
				cornSet.antialiasing = true;
				cornSet.scrollFactor.set(1, 1);
				cornSet.active = true;

				var cornSet2:FlxSprite = new FlxSprite(1050, 325).loadGraphic(Paths.image('extrabg/deadcorn'));
				cornSet2.antialiasing = true;
				cornSet2.scrollFactor.set(1, 1);
				cornSet2.active = true;				

				var farm:FlxSprite = new FlxSprite(-350, 450).loadGraphic(Paths.image('extrabg/cubes'));
				farm.antialiasing = true;
				farm.scrollFactor.set(0.98, 0.98);
				farm.active = true;

				var fence:FlxSprite = new FlxSprite(-350, 450).loadGraphic(Paths.image('extrabg/dead_fences'));
				fence.antialiasing = true;
				fence.scrollFactor.set(0.98, 0.98);
				fence.active = true;				

				var sign:FlxSprite = new FlxSprite(0, 500).loadGraphic(Paths.image('extrabg/nope'));
				sign.antialiasing = true;
				sign.scrollFactor.set(1, 1);
				sign.active = true;
