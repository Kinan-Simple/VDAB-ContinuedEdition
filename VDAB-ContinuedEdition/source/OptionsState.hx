...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Camera Note Movement':
						ClientPrefs.followarrow = !ClientPrefs.followarrow;

					case 'Eyesores And Flashing Lights':
					...
					case 'Enable Lane Underlay':
						ClientPrefs.laneunderlay = !ClientPrefs.laneunderlay;
			...
			if(controls.UI_LEFT || controls.UI_RIGHT) {
				...
				switch(options[curSelected]) {
					...
					case 'Lane Underlay':
						ClientPrefs.laneTransparency += add/10;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Enable Lane Underlay':
				daText = "Check for a underlay behind the notes";
			case 'Lane Underlay':
				daText = "Set lane underlay (Lane underlay must be enabled)";
			case 'Note Size':
				daText = "Size of notes and stuff";
			case 'Add Judgement Counter':
		     	daText = "Shows a judgement counter at the left of the screen (Example: Sicks: 3, Bads: 1)";
			case 'Note Splashes':
				daText = "If unchecked, hitting \"Sick!\" notes won't show particles.";
			case 'Eyesores And Flashing Lights':
				daText = "Uncheck this if you're sensitive to flashing lights and fast flashing colors!";
			case 'Enable Hitsounds':
				daText = "If checked, you will hear a click sound when hitting a note.";
			case 'Camera Note Movement':
				daText = "If checked it will enable a camera Movement on a Note Hit.";
			case 'Camera Zooms':
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Camera Note Movement':
						daValue = ClientPrefs.followarrow;
					case 'Eyesores And Flashing Lights':
					...
					case 'Enable Lane Underlay':
						daValue = ClientPrefs.laneunderlay;
					case 'Violence':
		...
		for (i in 0...grpTexts.members.length) {
			...
			if(text != null) {
				...
				switch(options[textNumber[i]]) {
					...
					case 'Lane Underlay':
						daText = ClientPrefs.laneTransparency+"";
				}
